  ...
  for i, v in pairs(oldchar:GetChildren()) do
    if v:IsA("BasePart") or v:IsA("Part") then
      if isnetworkowner(v) then
        if not newchar:FindFirstChild("FlingPart") then
          if v.Name ~= "Head" then
            v.Velocity = Unit(newchar[v.Name].Velocity, 16000) * 16000
    ...
    elseif v:IsA("Accessory") then
      ...
      if isnetworkowner(v.Handle) then
        ...
        v.Handle.Velocity = Unit(newchar[v.Name].Handle.Velocity, 16000) * 16000
      end
    end
  end
else
...
local LoadLibrary = loadstring(game:HttpGet("https://pastebin.com/raw/UfzKgS6T", true))()

local Create = LoadLibrary.Create
